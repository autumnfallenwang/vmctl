...
- [ ] Each codec frames its shape correctly, including multiline events kept whole.
- [ ] All three test-env log shapes (system text, route-capture multiline, audit json) produce correct ECS events (fields per ADR 0004).
- [ ] `@timestamp` and `labels.route_id` parse correctly per shape.
- [ ] NDJSON and human sinks both render an event.
- [ ] Unit tests run against the `testenv/corpus/` fixtures; check loop green.
